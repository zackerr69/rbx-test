-- ЧАСТЬ 1: ИКОНКА (ПРЯМОУГОЛЬНИК, ЦИФРЫ, СЕРЕБРО)
if _G.ZACK_FINAL_PART1 then return end
_G.ZACK_FINAL_PART1 = true
...
-- Прямоугольник (почти без скругления)
...
-- ЧАСТЬ 2: МЕНЮ (ОСНОВА, КНОПКИ, ВКЛ/ВЫКЛ)
if _G.ZACK_FINAL_PART2 then return end
_G.ZACK_FINAL_PART2 = true
...
menu.Size = UDim2.new(0, 420, 0, 600)
menu.Position = UDim2.new(0.5, -210, 0.5, -300)
...
-- Хранилище состояний
_G.ZACK_STATES = _G.ZACK_STATES or {}
_G.ZACK_MENU = menu
...
if _G.ZACK_FINAL_PART3 then return end
_G.ZACK_FINAL_PART3 = true

local container = _G.ZACK_CONTAINER
local player = game.Players.LocalPlayer
local runService = game:GetService("RunService")
...
createToggle("NOCLIP", Color3.fromRGB(150, 100, 50), function(state)
    _G.ZACK_STATES.noclip = state
end)

runService.Stepped:Connect(function()
    if _G.ZACK_STATES.noclip and player.Character then
...
print("✅ Часть 3: Основные функции")
-- ЧАСТЬ 4: INVISFLING
if _G.ZACK_FINAL_PART4 then return end
_G.ZACK_FINAL_PART4 = true
...
function createFlingWindow()
    ...
    flingWindow.Parent = _G.ZACK_GUI
...
createToggle("INVISFLING", Color3.fromRGB(200, 0, 100), toggleInvisFling)
-- ЧАСТЬ 5: ДРОЧКА
if _G.ZACK_FINAL_PART5 then return end
_G.ZACK_FINAL_PART5 = true
...
-- ЧАСТЬ 6: CHAMS 1–5
if _G.ZACK_FINAL_PART6 then return end
_G.ZACK_FINAL_PART6 = true
...
createToggle("CHAMS 3", Color3.fromRGB(255, 100, 200), function(state)
    if state then
        ...
        _G.ZACK_STATES.chams3Conn = conn
    else
        if _G.ZACK_STATES.chams3Conn then
            _G.ZACK_STATES.chams3Conn:Disconnect()
            _G.ZACK_STATES.chams3Conn = nil
        end
        resetLighting()
    end
end)
...
print("✅ Часть 6: Chams 1–5 готовы")
-- ЧАСТЬ 7: ФИНАЛ
if _G.ZACK_FINAL_PART7 then return end
_G.ZACK_FINAL_PART7 = true

game:GetService("StarterGui"):SetCore("SendNotification", {
    Title = "ZACK HUB",
    Text = "ФИНАЛЬНАЯ ВЕРСИЯ ЗАГРУЖЕНА",
    Duration = 3
})
